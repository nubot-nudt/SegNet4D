...
class LShapeFitting():
    ...
    def fitting(self, pointcluster):
        ...
        ox = pointcluster[:,0]
        oy = pointcluster[:,1]
        rect  = self._rectangle_search(ox,oy)
...
class RectangleData():
    ...
    def plot(self):
        _=self.calc_rect_contour()
        plt.plot(self.rect_c_x, self.rect_c_y, "-r")

    def calc_rect_contour_bev(self):
            ...
            print("==============================")
            print("x:",self.rect_c_x)
            print("y:",self.rect_c_y)
    ...
    def calc_rect_contour(self):
        ...
        self.center_x = np.mean(self.rect_c_x[:4], axis=0)
        self.center_y = np.mean(self.rect_c_y[:4], axis=0)

        min_x_index = np.argmin(self.rect_c_x[:4])
        max_x_index = np.argmax(self.rect_c_x[:4])

        min_y_index = np.argmin(self.rect_c_y[:4])
        max_y_index = np.argmax(self.rect_c_y[:4])
        # print("x:",self.rect_c_x[:4])
        # print("y:",self.rect_c_y[:4])
        # print("min_x_index:",min_x_index)
        # print("min_y_index:",min_y_index)
        # print("max_y_index:",max_y_index)

        assert (min_x_index == 3 or min_x_index ==0)
        assert min_y_index == 0
        assert max_y_index == 2

        
        if min_x_index!= min_y_index:
            l1 = np.sqrt((self.rect_c_x[min_x_index]-self.rect_c_x[min_y_index])*(self.rect_c_x[min_x_index]-self.rect_c_x[min_y_index])+\
                            (self.rect_c_y[min_x_index]-self.rect_c_y[min_y_index])*(self.rect_c_y[min_x_index]-self.rect_c_y[min_y_index]))

            l2 = np.sqrt((self.rect_c_x[min_x_index]-self.rect_c_x[max_y_index])*(self.rect_c_x[min_x_index]-self.rect_c_x[max_y_index])+\
                            (self.rect_c_y[min_x_index]-self.rect_c_y[max_y_index])*(self.rect_c_y[min_x_index]-self.rect_c_y[max_y_index]))
            ...
            if l1>=l2:
                self.length = l1
                self.width = l2
                self.theta =  - np.arctan((np.abs(self.rect_c_x[min_x_index]-self.rect_c_x[max_y_index])/np.abs(self.rect_c_y[min_x_index]-self.rect_c_y[max_y_index])))
            else:
                self.length = l2
                self.width = l1
                self.theta = np.pi/2 - np.arctan((np.abs(self.rect_c_x[min_x_index]-self.rect_c_x[max_y_index])/np.abs(self.rect_c_y[min_x_index]-self.rect_c_y[max_y_index])))
        else:#是个垂直的bounding box
            l1 = abs(self.rect_c_x[0] - self.rect_c_x[1])
            l2 = abs(self.rect_c_y[0] - self.rect_c_y[3])
            if l1>=l2:
                self.length = l1
                self.width = l2
                self.theta = 0
        ...
        self.bev[0] = self.center_x
        self.bev[1] = self.center_y
        self.bev[2] = self.length
        self.bev[3] = self.width
        self.bev[4] = self.theta
        return self.bev

        # if min_x_index == 0:
        #     l1 = np.sqrt((self.rect_c_x[0]-self.rect_c_x[1])*(self.rect_c_x[0]-self.rect_c_x[1])+(self.center_y[0]-self.rect_c_y[1])*(self.center_y[0]-self.rect_c_y[1]))
        #     l2 = np.sqrt((self.rect_c_x[0]-self.rect_c_x[3])*(self.rect_c_x[0]-self.rect_c_x[3])+(self.center_y[0]-self.rect_c_y[3])*(self.center_y[0]-self.rect_c_y[3]))
