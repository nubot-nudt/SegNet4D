...
class ClassificationMetrics(nn.Module):
    ...
    def compute_confusion_matrix(self, pred_logits: torch.Tensor, gt_labels: torch.Tensor):
        ...
        if self.n_classes!=pred_logits.shape[1]:
        ...
        confusion_matrix = confusion_matrix.index_put_(tuple(idxs), ones, accumulate=True) #二维Tensor不同行、不同列赋值，idx是坐标[0,0],[1,0],[1,1]...， ones意思是在对应的坐标下加上1，因为accumulate=True
