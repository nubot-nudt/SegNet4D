...
class nuScenesSequentialModule(LightningDataModule):
    ...
    def __init__(self, cfg):
        ...
        Returns:
          None
        """
        super(nuScenesSequentialModule, self).__init__()
        self.cfg = cfg
    ...
    def setup(self, stage=None):
        ...
        train_set = nuScenesSequentialDataset(self.cfg, split="train")

        val_set = nuScenesSequentialDataset(self.cfg, split="val")

        test_set = nuScenesSequentialDataset(self.cfg, split="test")
...
class nuScenesSequentialDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # load bounding box 
        bounding_box_file = [ os.path.join(self.root_dir, str(seq).zfill(4), "boundingbox", str(i).zfill(6) + ".npy")
            for i in past_indices ]

        data_dict = {}
 
        if self.online_train:
            list_bounding_box = [self.read_bounding_nuscene_box_label(bounding_box_file[-1])] 
            ...
            # Load past labels
            label_files = [
                os.path.join(self.root_dir, str(seq).zfill(4), "labels", str(i).zfill(6) + ".label")
                for i in past_indices
            ]

            past_labels = [self.read_nuscence_mos_labels(f) for f in label_files]
            semantic_labels = [self.read_nuscence_semantic_labels(f) for f in label_files]
            ...
            mos_labels = self.read_nuscence_mos_labels(label_files[-1])
            semantic_labels = self.read_nuscence_semantic_labels(label_files[-1])
            ...
            if self.training and self.cfg["MODEL"]["OBJECT_DETECTION"]:
                gt_box_for_augment = gt_box[:,0:7]
            ...
            elif self.training and self.cfg["MODEL"]["OBJECT_DETECTION"]==False:
                past_points_for_transform = current_point_with_feature[:,:3]
                past_points_for_transform = self.augment_data(past_points_for_transform)
                current_point_with_feature[:,:3] = past_points_for_transform
    ...
    def read_nuscence_mos_labels(self, filename):
        """Load moving object labels from .label file"""
        if os.path.isfile(filename):
            labels = np.fromfile(filename, dtype=np.uint8)
    ...
    def read_nuscence_semantic_labels(self, filename):
        """Load moving object labels from .label file"""
        if os.path.isfile(filename):
            labels = np.fromfile(filename, dtype=np.uint8)
            labels = labels.reshape((-1))
    ...
    def read_bounding_nuscene_box_label(self,filename):
