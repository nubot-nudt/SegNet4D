...
def random_flip(pointcloud_with_batch,gt_boxes):
  
  points = pointcloud_with_batch[:,0:3]
  ...
  if flip_type==1:
    ...
    gt_boxes,points = random_flip_along_z(gt_boxes,points)
...
def jitter_point_cloud(point_cloud, sigma=0.01, clip=0.05):
    ...
    jitter = np.clip(sigma * np.random.randn(N, 3), -1 * clip, clip)
...
def random_elastic_aug(pointcloud,elastic_gran=[0.12, 0.4],elastic_mag=[0.8, 3.2]):
    pointcloud[:, 0:3] =  elastic(pointcloud[:, 0:3],elastic_gran[0],elastic_mag[0])
    pointcloud[:, 0:3] =  elastic(pointcloud[:, 0:3],elastic_gran[1],elastic_mag[1])
